from typing import List, Dict
from bs4 import BeautifulSoup
from .interfaces.html_collector_interface import HtmlCollectorInterface

class HtmlCollector(HtmlCollectorInterface):

    def collect_essencial_information(self, html:str) -> List[Dict[str, str]]:
        soup = BeautifulSoup(html, 'html.parser')

        artist_name_list = soup.find(class_='BodyText')
        artist_name_items = artist_name_list.find_all('a')

        essencial_information = []
        for artist_name in artist_name_items:
            names = artist_name.contents[0]
            links = 'https://web.archive.org'+artist_name.get('href')
            essencial_information.append({
                'name': names,
                'link': links
            })

        return essencial_information
